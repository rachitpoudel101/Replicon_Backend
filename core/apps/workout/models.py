...
class TrainerMember(models.Model):
    ...
    )  # limit choice_to ensures only users with role 'trainer' can be selected.
    member = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name="member_trainer",
        limit_choices_to={"role": "member"},
    )  # limit choice_to ensures only users with role 'member' can be selected.
...
class WorkoutPlanExercise(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True
    )  # Special instructions for this exercise
...
class MemberProgress(models.Model):
    ...
    measurements = models.JSONField(
        blank=True, null=True
    )  # Store chest, waist, hip measurements
    progress_photos = models.ImageField(
        upload_to="progress_photos/", blank=True, null=True
    )
    notes = models.TextField(blank=True, null=True)
